...
const DEV = false; // set false for production
...
/* Waves state (multi-letter per slot) */
let slotQueues = new Map(); // slotId -> [cellKey1, cellKey2, ...]
let slotCursor = new Map(); // slotId -> next index to emit
let slotActive = new Map(); // slotId -> currently displayed cellKey
...
async function newPuzzle() {
  try {
    ...
    if (pool && pool.puzzles.length) {
      ...
      out = {
        ...
        slotAssignment: {
          ...
          // optional in older pools; guard in renderer
          slotQueues: p.slotAssignment.slotQueues ? toMap(p.slotAssignment.slotQueues) : undefined
        }
      };
    } else {
    ...
    if (DEV) {
      ...
      console.log('Outside slot assignment (slotId -> cell or queue):', slotAssignment);
    }
...
function shiftSlotAssignmentKeys(assignment, padTop, padLeft) {
  ...
  for (const [key, info] of assignment.byCell.entries()) {
    ...
    if (slotQueues) {
      // rebuild queue entries into shifted ids
      // collect from original assignment.slotQueues if present
...
/* ===== Token rendering with waves support ===== */

/** Create and attach a token element for a given cellKey into its slot */
...
// NEW: wave tagging (0 = original wave)
  const wave = Number(info.wave ?? 0);
  tokenEl.dataset.wave = String(wave);           // used by CSS
  if (wave > 0) tokenEl.classList.add('wave');   // generic flag for later waves
  ...
  if (slotEl) {
    ...
    slotActive.set(info.id, cellKey);
  }
...
function emitNextTokenIntoSlot(slotId) {
  if (!slotQueues.size) return; // non-wave fallback
  const queue = slotQueues.get(slotId) || [];
  const cursor = slotCursor.get(slotId) || 0;
  if (cursor >= queue.length) {
    // No more tokens for this slot
  ...
  const info = slotAssignment.byCell.get(cellKey);
  const letter = solutionLetters.get(cellKey);
  slotCursor.set(slotId, cursor + 1);
...
function renderTokensFromAssignment(letters, assignment) {
  ...
  // Waves state
  slotQueues = new Map(assignment.slotQueues || []);
  slotCursor = new Map();
  slotActive = new Map();

  if (slotQueues.size) {
    // Waves: only render first of each queue
    for (const [slotId, q] of slotQueues.entries()) {
      if (!q.length) continue;
      const firstCell = q[0];
      const info = assignment.byCell.get(firstCell);
      const letter = letters.get(firstCell);
      slotCursor.set(slotId, 1); // next index to emit
      createTokenForCell(firstCell, info, letter);
    }
  } else {
    // Fallback: original behavior (single letter per slot)
...
function onBoardCellClick(e) {
  ...
  if (existingTokenId) {
    ...
    if (slotEl) {
      // If waves are active and another token is occupying the slot, roll back one emission:
      if (slotQueues.size) {
        const activeId = slotActive.get(tok.slotId);
        // If a different active token is currently shown, remove it and rewind cursor
        if (activeId && activeId !== tok.id) {
          const activeTok = tokens.get(activeId);
          activeTok?.el?.remove();
          const cur = slotCursor.get(tok.slotId) || 0;
          slotCursor.set(tok.slotId, Math.max(0, cur - 1));
        }
        slotActive.set(tok.slotId, tok.id);
      }
      slotEl.classList.remove('empty');
      slotEl.classList.add('occupied');
  ...
  // Remove token from its slot; slot becomes empty (temporarily)
  ...
  // Refill with next token for this slot if waves are active
  emitNextTokenIntoSlot(tok.slotId);
